...
import { getScreenshot, setLabel } from "features/auth/authSlice"
import * as faceapi from 'face-api.js'
import { detectFaces, getStyles, matchFace, setFacenetError, setStyles } from "features/auth/facenetSlice"
...
export const Preview = (props) => {
    ...
    const handleScreenshot = async (blob) => {
        ...
        let pic1 = await baseToFormData(blob, 'file');
        const formData = new FormData();
            formData.append('file', pic1);
        
     let labelName = await dispatch(matchFace(formData))
        .then(({payload}) => {
            if(payload && payload.response && payload.response.status === 404){
                return 'Face not recognized!'
            } else {
                let match = payload && payload.results ? payload.results[0] : {label: "Face not recognized!"};
                return match.label;
            }
        })
        .catch(err => {
         ...
         dispatch(detectFaces({previewRef, canvasRef})).then(r => {
            ...
            if (faces && faces.length > 0) {
                let options = {
                    lineWidth: 4,
                    boxColor: (faces.length === 1 && faces[0].detection.score > 0.50) ? '#00ff00' : '#f00000',
